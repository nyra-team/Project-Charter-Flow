...
export function ClosureReadinessSection({ projectId }: { projectId: number }) {
  ...
  const csatColor = csatScore >= 4 ? "#059669" : csatScore >= 3 ? "#D97706" : "#DC2626";
  ...
  return (
    ...
    >
      <div>
        <p className="text-sm font-bold text-sky-900">Closure Readiness Assessment</p>
      ...
      <div>
        <p className="text-xs font-bold text-sky-800 mb-2">Handover Checklist</p>
        <div className="space-y-2">
          {HANDOVER_ITEMS.map((item) => (
            <button
              ...
              style={{
                background: handover[item.id] ? "#ECFDF5" : "white",
                border: `1px solid ${handover[item.id] ? "#86EFAC" : "#E0F2FE"}`,
              }}
            >
              {handover[item.id] ? (
                <CheckCircle2 size={14} className="text-emerald-500 flex-shrink-0" />
              ) : (
                <Circle size={14} className="text-sky-300 flex-shrink-0" />
              )}
              <span
                className={`text-xs flex-1 ${handover[item.id] ? "text-emerald-800 line-through" : "text-gray-700"}`}
              >
                {item.label}
              </span>
            </button>
      ...
      <div>
        <p className="text-xs font-bold text-sky-800 mb-2">Customer Satisfaction (CSAT)</p>
        <div className="space-y-2">
          <div className="flex items-center gap-3">
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                className="rounded"
              />
              <span className="text-xs text-gray-700">CSAT survey distributed and complete</span>
            </label>
          </div>

          <div className="flex items-center gap-3">
            <label className="text-xs text-gray-600 w-32 flex-shrink-0">Response count</label>
            <input
              type="number"
              min={0}
              value={csatResponses}
              onChange={(e) => setCsatResponses(Math.max(0, Number(e.target.value)))}
              className="w-20 text-sm border border-sky-200 rounded-lg px-2 py-1 bg-white outline-none focus:ring-1 focus:ring-sky-400"
            />
          </div>

          <div>
            <div className="flex items-center justify-between mb-1">
              <label className="text-xs text-gray-600">Average CSAT score</label>
              <span
      ...
      {readyToClose ? (
        ...
        <div className="rounded-xl p-3" style={{ background: "#FFF7ED", border: "1px solid #FED7AA" }}>
          <p className="text-xs font-semibold text-orange-800">Not yet ready for closure:</p>
