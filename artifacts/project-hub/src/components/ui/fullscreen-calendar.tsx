...
export function FullScreenCalendar({ data }: FullScreenCalendarProps) {
  ...
  // Week-rows in this month (28/35/42 days → 4/5/6) so EVERY week renders on one
  // screen — no fixed 5-row clipping for months that span six weeks.
  const weekRows = Math.ceil(days.length / 7)

  // Month-picker options — a ±2-year window around today.
  const baseYear = today.getFullYear()
  const monthOptions: { value: string; label: string }[] = []
  for (let y = baseYear - 2; y <= baseYear + 2; y++) {
    for (let m = 0; m < 12; m++) {
      const d = new Date(y, m, 1)
      monthOptions.push({ value: format(d, "MMM-yyyy"), label: format(d, "MMMM yyyy") })
    }
  }
    ...
    <div className="flex flex-1 flex-col">
      ...
      <div className="flex flex-col space-y-4 p-4 md:flex-row md:items-center md:justify-between md:space-y-0 lg:flex-none">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
              <select
                ...
                className="-ml-1 w-fit cursor-pointer rounded-md border border-input bg-background px-2 py-0.5 text-lg font-semibold text-foreground hover:bg-accent focus:outline-none focus:ring-2 focus:ring-ring/30"
              >
                {monthOptions.map((o) => (
        ...
        <div className="flex flex-col items-center gap-4 md:flex-row md:gap-6">
          {/* Read-only project calendar — month navigation only (events are
              derived from each project's start / end dates). */}
      ...
      <div className="lg:flex lg:flex-auto lg:flex-col">
        ...
        <div className="flex text-xs leading-6 lg:flex-auto">
          <div
            className="hidden h-full w-full border-x lg:grid lg:grid-cols-7"
