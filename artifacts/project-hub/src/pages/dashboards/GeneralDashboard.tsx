...
import { useAutoRefresh } from "../../components/dashboard/primitives";
...
function MetricCard({
  ...
  const inner = (
    <div className="rounded-2xl p-5 flex items-start justify-between group cursor-pointer transition-all hover:shadow-md hover:-translate-y-0.5" style={{ background: "white", border: "1px solid #E2E8F0" }}>
      <div>
        <p className="text-xs font-semibold uppercase tracking-widest text-gray-400 mb-1">{label}</p>
        <div className="text-2xl font-bold text-gray-900">{value}</div>
        {sub && <p className="text-xs text-gray-400 mt-1">{sub}</p>}
      </div>
      <div className="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform" style={{ background: gradient }}>
...
function ProjectHealthSection({ health }: { health: DashboardSummary["projectHealth"] }) {
  ...
  const tiles = [
    { label: "Total Active", value: health.active ?? 0, icon: BarChart3, color: "#6366F1", bg: "#EEF2FF", textColor: "#4338CA" },
    { label: "On Track", value: health.onTrack ?? 0, icon: CheckCircle2, color: "#10B981", bg: "#ECFDF5", textColor: "#065F46" },
    { label: "Off Track", value: health.offTrack ?? 0, icon: AlertTriangle, color: "#F59E0B", bg: "#FFFBEB", textColor: "#92400E" },
    { label: "Delayed", value: health.delayed ?? 0, icon: XCircle, color: "#EF4444", bg: "#FEF2F2", textColor: "#991B1B" },
  ];
  ...
  return (
    <div className="rounded-2xl p-5 h-full" style={{ background: "white", border: "1px solid #E2E8F0" }}>
        ...
        {tiles.map(t => {
          ...
          return (
            <div key={t.label} className="rounded-xl p-3 text-center" style={{ background: t.bg }}>
              <div className="flex justify-center mb-1"><Icon size={18} style={{ color: t.color }} /></div>
        ...
        <div className="space-y-2">
          ...
          {problems.map((p, i) => (
            <Link key={i} href={`/projects/${p.id}`}>
              <div className="flex items-start gap-3 p-3 rounded-xl hover:opacity-80 transition-opacity cursor-pointer" style={{ background: p.kind === "delayed" ? "#FEF2F2" : "#FFFBEB" }}>
                {p.kind === "delayed" ? <XCircle size={15} className="text-red-500 mt-0.5 flex-shrink-0" /> : <AlertTriangle size={15} className="text-amber-500 mt-0.5 flex-shrink-0" />}
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-gray-800 truncate">{p.name}</p>
                  <p className="text-xs text-gray-500 truncate">{p.reason}</p>
                </div>
                {p.kind === "delayed" && p.daysOverdue != null && <span className="text-xs font-bold text-red-600 flex-shrink-0">+{p.daysOverdue}d</span>}
                {p.kind === "off-track" && p.behindBy != null && <span className="text-xs font-bold text-amber-600 flex-shrink-0">{p.behindBy}% behind</span>}
              </div>
            </Link>
          ))}
        </div>
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          {data.leaderboard.slice(0, 5).map((user, idx) => (
            <div key={user.userId} className="flex items-center gap-3 p-3 rounded-xl" style={{ background: idx === 0 ? "linear-gradient(135deg, #FFFBEB, #FEF3C7)" : "#F8FAFC", border: idx === 0 ? "1px solid #FCD34D" : "1px solid #E2E8F0" }}>
              <div className="text-xl flex-shrink-0">{RANK_EMOJIS[idx] ?? `${idx + 1}.`}</div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
                  <p className="text-sm font-semibold text-gray-800 truncate">{user.name}</p>
                  {idx === 0 && <span className="text-xs px-1.5 py-0.5 rounded font-medium" style={{ background: "#FEF3C7", color: "#92400E" }}>{RANK_LABELS[0]}</span>}
                </div>
...
export default function GeneralDashboard() {
  ...
  const charterStatusData = summary?.chartersByStatus?.map((item, i) => ({
    ...item, fill: STATUS_COLORS[item.status] ?? FALLBACK_COLORS[i % FALLBACK_COLORS.length],
  })) ?? [];
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href="/charters/new">
            <button className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold text-white transition-all hover:opacity-90 active:scale-95" style={{ background: "linear-gradient(135deg, #6366F1, #8B5CF6)" }}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        <MetricCard label="Total Charters" value={summary?.totalCharters ?? 0} icon={FileText} gradient="linear-gradient(135deg,#6366F1,#8B5CF6)" href="/charters" sub="All time" />
        <MetricCard label="Pending Approvals" value={summary?.pendingApprovals ?? 0} icon={CheckSquare} gradient="linear-gradient(135deg,#F59E0B,#F97316)" href="/approvals" sub="Awaiting action" />
        <MetricCard label="Active Projects" value={summary?.activeProjects ?? 0} icon={TrendingUp} gradient="linear-gradient(135deg,#10B981,#059669)" href="/projects" sub="In execution" />
        <MetricCard label="Approved Budget" value={formatCurrency(summary?.totalBudgetApproved ?? 0)} icon={DollarSign} gradient="linear-gradient(135deg,#3B82F6,#1D4ED8)" sub="Total approved" />
      </div>
            ...
            {charterStatusData.length > 0 ? (
              <>
                <ResponsiveContainer width="100%" height={180}>
                  <PieChart>
                    <Pie data={charterStatusData} cx="50%" cy="50%" innerRadius={50} outerRadius={75} paddingAngle={3} dataKey="count" nameKey="status">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-5 gap-5">
        ...
        <div className="xl:col-span-3 rounded-2xl p-5 h-full" style={{ background: "white", border: "1px solid #E2E8F0" }}>
          <div className="mb-4">
            ...
            <div className="space-y-0 divide-y divide-gray-50">
              {activities.slice(0, 7).map((activity) => (
                <div key={activity.id} className="flex items-start gap-3 py-3 first:pt-0 last:pb-0">
                  <div className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5" style={{ background: "linear-gradient(135deg, #6366F1, #8B5CF6)" }}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      <span className="text-xs text-gray-400">{format(new Date(activity.createdAt), "MMM d, h:mm a")}</span>
                      {activity.userName && <><span className="text-gray-300">·</span><span className="text-xs text-gray-400">{activity.userName}</span></>}
                    </div>
                  </div>
                </div>
              ))}
