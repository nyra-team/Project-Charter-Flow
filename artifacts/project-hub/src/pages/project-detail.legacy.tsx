...
export default function ProjectDetail() {
  ...
  // Active projects open with the Business Case stage selected so the Plan
  // phase is visible without a click. Runs once per visit; a ?stage= deep-link
  // wins, and the user can still deselect or pick any other stage afterwards.
  ...
  useEffect(() => {
    if (stageDefaultApplied.current || !project) return;
    stageDefaultApplied.current = true;
    if (new URLSearchParams(search).get("stage")) return;
    if (project.status === "active") {
      setSelectedStageKey((prev) => prev ?? "initiation");
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [project]);
