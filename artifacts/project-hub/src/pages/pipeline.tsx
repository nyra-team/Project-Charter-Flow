...
// A "gate" inside a phase — the user-facing decision points. Initiate exposes its
// two sub-gates (BC, URS); every other phase exposes its constituent stages.
...
function gatesForPhase(phaseKey: PhaseKey): Gate[] {
  if (phaseKey === "initiate") {
    const init = getStageConfig("initiation");
    return [
      { id: "business_case", label: "Business Case", shortLabel: "BC", stageKey: "initiation", color: init?.color ?? "#6366F1" },
      { id: "urs", label: "URS", shortLabel: "URS", stageKey: "initiation", color: init?.color ?? "#6366F1" },
    ];
  }
